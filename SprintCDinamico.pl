...
gera:-
    ...
    /*
    verifica_lista_remover(LR),
    verifica_lista_adicionar(LA),
    verifica_dimensao_aumentada(AD),
    */
...
alteracoes_dinamicas :-
    ...
    write('Lista que pretende remover: '),
    read(LR), 
    (retract(remove_lista(_));true), asserta(remove_lista(LR)),

    write('Lista que pretende adicionar: '),
    read(LA), 
    (retract(adiciona_lista(_));true), asserta(adiciona_lista(LA)), 

    write('Alteração da Dimensão da entrega: '), 
    read(AD),
    (retract(append_aumenta_dimensao(_));true), asserta(append_aumenta_dimensao(AD)).

remove_lista([], _, []).

remove_lista([LR|CaudaLista], SegundaLista, Resultado):- 
    member(LR, SegundaLista), !, remove_lista(CaudaLista, SegundaLista, Resultado). 

remove_lista([LR|CaudaLista], SegundaLista, [LR|Resultado]):- 
    remove_lista(CaudaLista, SegundaLista, Resultado).

adiciona_lista([],L,L).

adiciona_lista([LA|CaudaLista],SegundaLista,[LA|Resultado]):-
    adiciona_lista(CaudaLista,SegundaLista,Resultado).

aumenta_dimensao([AD|Lista1],[Y|Lista2],[AD,Y|Pop]) :- 
    aumenta_dimensao(Lista1,Lista2,Pop).

append_aumenta_dimensao([],[], L).

append_aumenta_dimensao(AD, MViagem, Pop).

verifica_lista_remover(0) :-  remove_lista.
verifica_lista_remover(LR) :- \+(LR = 0), obterPopulacao.

verifica_lista_adicionar(0) :-  adiciona_lista.
verifica_lista_adicionar(LA) :- \+(LA = 0), obterPopulacao.

verifica_dimensao_aumentada(0) :- append_aumenta_dimensao.
verifica_dimensao_aumentada(AD) :- \+(AD = 0), obterPopulacao.
