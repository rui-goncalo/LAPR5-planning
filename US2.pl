...
% --- Solução ---
/*
obterCargaCamiao(Data, Lista de Armazens, Lista de Cargas, Carga Total do Camião)

Calcula o peso total de carga de um camião em uma determinada data.
*/
...
Calcula a massa da carga do próximo armazém e adiciona à carga total.
Chama recursivamente a função para o próximo armazém e carga na lista.
*/
obterCargaCamiao(Data, [A|LA], [C|LC], C) :-
    obterCargaCamiao(Data, LA, LC, C1),
    entrega(_, Data, MassaEntrega, A, _, _),
    C is MassaEntrega + C1.

/*
adicionaTaraCamiao(Truck, Lista de Cargas, Tara+Carga)

Vai buscar a tara de um Truck para no adicionaTara proceder a cálculos.
*/
adicionaTaraCamiao(T, LC, TC) :-
        carateristicasCam(T, Tara, _, _, _, _),
        adicionaTara(Tara, LC, TC).

/*
adicionaTara(Tara de um Truck, Lista de Cargas, Lista Cargas+Tara).

Calcula a soma das cargas e da tara do Truck que recebe de cima.
*/
adicionaTara(Tara, [], [Tara]).
adicionaTara(Tara, [C|LC], [TaraC|TC]):-
        TaraC is C + Tara,
        adicionaTara(Tara, LC, TC).

/*
calcularTempo(Data, Truck, Lista de Armazéns, Tempo).

Calcula o tempo total de uma viagem de um camião em uma determinada data.
Como condicionantes tem de calcular a carga do camião
*/
calcularTempo(Data, T, LA, Tempo) :-
        obterCargaCamiao(Data, LA, LC, _),
        adicionaTaraCamiao(T, LC, TC),
        idArmazem('Matosinhos', AId),
        append([AId|LA], [AId], Rota),
        carateristicasCam(T, Tara, CapacCarga, B, _, TempoCarreg),
        Capac is Tara + CapacCarga,
        tempoRota(Rota, TC, Capac, B, B, TempoCarreg, Data, Tempo).

/*
tempoRota(Rota, Tara+Carga, Capacidade do Camião, Bateria Atual, Bateria Completa, Tempo de Carregamento, Data, Tempo).

Ele recebe como entrada a rota, uma lista de cargas para cada armazém na rota, a capacidade do camião,
a bateria atual do camião, a bateria cheia do camião, o tempo de carregamento da bateria, a data e devolve 
como saída o tempo total da rota.
*/

tempoRota([A1, A2], [C1], Capac, B, BFull, _, _, Tempo) :-
        dadosCam_t_e_ta(_, A1, A2, BatMax, Autonomia, TempoExtra),
        Viagem is BatMax * C1 / Capac,
        EnergiaGasta is Autonomia * C1 / Capac,
        EnergiaRest is B - EnergiaGasta,
        ((EnergiaRest < (BFull * 0.2), TempoCarregamento is TempoExtra);
        (TempoCarregamento is 0)),
        Tempo is Viagem + TempoCarregamento.

tempoRota([A1, A2, A3 | Rota], [C1, C2 | TC], Capac, B, BFull, TempoCarreg, Data, Tempo) :-
    dadosCam_t_e_ta(_, A1, A2, BatMax, Autonomia, TempoExtra),
    Viagem is BatMax * C1 / Capac,
    EnergiaGasta is Autonomia * C1 / Capac,
    EnergiaRest is B - EnergiaGasta,
    ((EnergiaRest < (BFull * 0.2) -> 
        BatChegada is (BFull * 0.2), 
        TempoCarregamento is TempoExtra);
    (BatChegada is EnergiaRest, TempoCarregamento is 0)),
    entrega(_, Data, _, A2, _, TempoDescarga),
    dadosCam_t_e_ta(_, A2, A3, _, GastoBatProxArm, _), 
    BatNecessaria is GastoBatProxArm * C2 / Capac,
    idArmazem('Matosinhos', AId),
    ((A3 == AId, BatChegada - BatNecessaria < (BFull * 0.2) ->
        Carregamento is ((BFull * 0.2) - (BatChegada - BatNecessaria)),
        TempoCarregamentoBat is Carregamento * TempoCarreg / (BFull * 0.6),
    ProxCarregamBat is Carregamento + BatChegada);
    (((BatNecessaria > BatChegada ->
        ProxCarregamBat is (BFull * 0.8),
        TempoCarregamentoBat is ((BFull * 0.8) - BatChegada) * TempoCarreg / (BFull * 0.6));
    ((BatChegada - BatNecessaria < (BFull * 0.2) ->
        ProxCarregamBat is (BFull * 0.8),
        TempoCarregamentoBat is ((BFull * 0.8) - BatChegada) * TempoCarreg / (BFull * 0.6));
    (ProxCarregamBat is EnergiaRest, TempoCarregamentoBat is 0))))),
    ((TempoCarregamentoBat > TempoDescarga ->
        TempoEspera is TempoCarregamentoBat);
    (TempoEspera is TempoDescarga)),
    tempoRota([A2, A3 | Rota], [C2 | TC], Capac, ProxCarregamBat, BFull, TempoCarreg, Data, Tempo1),
    Tempo is Tempo1 + Viagem + TempoCarregamento + TempoEspera.

run(Data, T):-
    retractall(tempo_min(_, _)),
    assertz(tempo_min(_, 100000)),
    findall(AId, entrega(_, Data, _, AId, _, _), LA),
    permutation(LA, LAP),
    calcularTempo(Data, T, LAP, Tempo),
    atualiza(LAP, Tempo),
    fail.
...
atualiza(LF, Tempo) :- 
        tempo_min(_, Min),
        ((Tempo < Min, !, 
            retract(tempo_min(_, _)),
            assertz(tempo_min(LF, Tempo))); 
        true).

/*melhorViagem(Data, T, L, Tempo):-
        get_time(Ti),
        (run(Data, T); true),
        tempo_min(L, Tempo),
        get_time(Tf), 
        TResult is Tf-Ti,
        write(TResult),nl, 
        format('Tempo de execucao mais rapido: ~3f~n', [TResult]),
        format('Viagem mais rapida encontrada: ~3f~n', [Tempo]).*/

melhorViagem(Data, T, L, Tempo):- (run(Data, T); true), tempo_min(L, Tempo).