...
% --- US 3.2.3 - 1.c - Aumentar a dimensão do problema (colocando mais armazéns a visitar) e verificar até que dimensão é viável proceder do modo adotado (com um gerador de todas as soluções) efetuando um estudo de complexidade do problema.

armazensEntregasData(L, D) :- findall(AID, entrega(_, D, _, AID, _, _), L).
todosCaminhos(LC, D) :- armazensEntregasData(LF, D), findall(R, permutation(LF, R), LC).
caminho(LL, D) :- todosCaminhos(LC, D), caminhoTotal(LC, LL).
caminhoTotal([], []).
caminhoTotal([H|LC], [HH|LL]):- adicionaArmazemPrincipal(H, HH), write(HH), nl, caminhoTotal(LC, LL).
adicionaArmazemPrincipal(L, LL) :- idArmazem('Matosinhos', ABId), append([ABId|L], [ABId], LL).

solucoesTotais([_], 0):- !.
solucoesTotais([_|LC], Y):- solucoesTotais(LC, X), Y is X + 1.

caminhos(LL, D) :- 
	get_time(Ti),
    todosCaminhos(LC, D),
	solucoesTotais([_|LC], Y),
  	caminhoTotal(LC, LL),
	caminho(LL, D),
    get_time(Tf), nl, nl,  
  	write(Y), write(' : Solucoes encontradas em '),
  	T is Tf-Ti, write(T), write(' segundos.'), nl, nl,
